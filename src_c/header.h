...
typedef enum {
    ...
    TOKEN_STRING, TOKEN_SHOW, TOKEN_COLON,
    TOKEN_ID, TOKEN_LET,
    ...
    TOKEN_RUN,       // Keyword 'run' to start async event loop
    TOKEN_TRY, TOKEN_CATCH, TOKEN_AS, TOKEN_FINALLY, // New for try-catch
    TOKEN_RAISE,                                    // New for raise

    TOKEN_IF, TOKEN_ELIF, TOKEN_ELSE,
    TOKEN_LOOP, TOKEN_WHILE, TOKEN_FOR, TOKEN_FROM, TOKEN_TO, TOKEN_STEP, TOKEN_IN,
    TOKEN_BREAK, TOKEN_CONTINUE, TOKEN_END,
...
typedef struct Function {
    ...
    int body_end_token_original_line;
    int body_end_token_original_col;
...
typedef enum {
    ...
    CORO_RUNNING,  // Currently executing by the event loop (transient state)
...
typedef struct Coroutine {
    ...
    LexerState resume_state;    // Lexer state to resume from
    ...
    int is_resumed_from_await;  // Flag: 1 if just resumed after an await
...
struct InterpreterImpl {
    ...
    int coroutine_yielded_for_await; // Flag set by await to signal event loop
...
// Debugging Macro
#ifdef DEBUG_ECHOC
extern FILE* echoc_debug_log_file; // Declare global log file pointer
...
#define ECHOC_MAX_LOG_FILE_SIZE (2 * 1024 * 1024) // MiB
#define ECHOC_LOG_TRUNCATE_THRESHOLD (ECHOC_MAX_LOG_FILE_SIZE - (16 * 1024)) // Reset if within 16KB of limit
#define BUG_PRINTF(format, ...) log_debug_message_internal(__FILE__, __LINE__, __func__, format, ##__VA_ARGS__)
