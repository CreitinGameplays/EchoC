...
const char* token_type_to_string(TokenType type) {
    switch (type) {
        ...
        case TOKEN_SHOW: return "SHOW_KEYWORD ('show')";
        ...
        case TOKEN_END: return "END_KEYWORD ('end')";
        ...
        case TOKEN_RUN: return "RUN_KEYWORD ('run')";
...
void interpreter_eat(Interpreter* interpreter, TokenType expected_type) {
    if (interpreter->current_token->type == expected_type) {
        ...
        sprintf(error_message, "Expected token %s, but got %s (value: '%s')",
